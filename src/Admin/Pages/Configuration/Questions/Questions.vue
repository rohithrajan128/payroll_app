<template>
    <div class="main-20">
        <div flat color="white" class="row py-5 pl-5 align-items-center">
            <page-title class="col-md-4" :heading="$t('questions')" :google_icon="google_icon"></page-title>
            <div class="col-md-6">
                <v-tooltip bottom>
            ...
            <div class="col-md-2">
                <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                        <router-link class="text-decoration-none" :to="{ name: 'questions.amend',
                        query:{ sequence:next_seq},
                         }"
                           >
        ...
        <v-data-table :headers="headers" :items="questions_array" :loading="initval" :search="search">
            <template v-slot:item="props">
                <tr>
                    <td>
                        <span v-if="props.item.role_type_id">{{ props.item.fetch_role_name.rolename }}</span>
                        <span v-else>{{ $t('not_appllicable') }}</span>
                    </td>
                    <td>
                        <v-tooltip bottom v-if="props.item.questions" max-width="370" color="white">
                            <template v-slot:activator="{ on }">
                                <div class="description_hover_div" v-on="on">
                                    {{ props.item.questions }}
                                </div>
                            </template>
                            <v-sheet class="pa-4 b">{{ props.item.questions }}</v-sheet>
                        </v-tooltip>
                        <span v-else>{{ $t('not_appllicable') }}</span>
                    </td>
                    <td>
                        <span v-if="props.item.fetchsection">{{ props.item.fetchsection.section_name }}</span>
                        <span v-else>{{ $t('not_appllicable') }}</span>
                    </td>
                    <td>
                        <span v-if="props.item.field_type">{{ props.item.field_type }}</span>
                        <span v-else>{{ $t('not_appllicable') }}</span>
                    </td>
                    <td>
                        <span v-if="props.item.is_field_required">{{ props.item.is_field_required }}</span>
                        <span v-else>{{ $t('not_appllicable') }}</span>
                    </td>
                    <td>
                        <span v-if="props.item.sequence">{{ props.item.sequence }}</span>
                        <span v-else>{{ $t('not_appllicable') }}</span>
                    </td>
                    <td>
                        <v-tooltip bottom>
                    ...
                    <td class="text-center">
                        ...
                        <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-icon class="mr-2 edit_btn icon_size" @click="deleteQuestion(props.item.id)"
                                    color="error" v-on="on">mdi-trash-can-outline</v-icon>
                                </template>
                                <span>{{ $t("delete") }}</span>
                            </v-tooltip>
                    </td>
...
export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            },
            {
                text: "Section",
                align: "left",
                value: "fetchsection.section_name"
            },
    ...
    methods: {
        ...
        fetchQuestions() {
            this.initval = true;
            axios
                .get(process.env.VUE_APP_API_URL_ADMIN + "fetchallquestions/" +
                    this.user_id)
                .then(res => {
        ...
        Math,
        this.questions_array.map(function (o) {
          return o.sequence;
        })
      );
      var newval = max_seq > 0 ? max_seq + 1 : 1;
      this.next_seq = newval;
                })
                .catch(err => {
