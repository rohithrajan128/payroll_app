<template>
  <div class="main-20">
    <div flat color="white" class="row py-5 pl-5 align-items-center">
      <page-title class="col-md-4" :heading="$t('noc')" :google_icon="google_icon"></page-title>
...
<td>
    <!-- query: {
            ...
            <router-link  class="text-decoration-none" :to="{
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-chip v-on="on" color="success" small outlined medium class="userindexcodechip">{{item.emp_details.empno
                    }}</v-chip>
                  </template>
                  <span>{{ $t("view") }}</span>
                </v-tooltip>
...
export default {
  ...
  methods: {
    ...
    fetchcategories() {
      this.initval = true;
      axios
        .get(process.env.VUE_APP_API_URL_ADMIN + "get_manager_noc")
        .then(res => {
