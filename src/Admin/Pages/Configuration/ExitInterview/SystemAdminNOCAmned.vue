              ...
              <div v-if="item.field_type == 'RADIO BUTTON'">
                ...
                >
                  <v-radio
                    ...
                    v-for="(lookupchild, checkindex) in  item.lookup_values"
                    :key="checkindex"
                    :label="lookupchild"
                    :value="lookupchild"
                  ></v-radio>
  ...
  watch: {
    "$route.query.slug": {
      ...
      handler() {
        ...
        if (this.user_slug) {
          this.fetchSystemAdminQuestions();
        }
      }
    }
  },
  ...
  methods: {
    saveHranswers() {
      if (this.$refs.answerform.validate()) {
        ...
        axios
          .post(
            process.env.VUE_APP_API_URL_ADMIN + "saveemployeeanswerfromsystemadmin",
            {
