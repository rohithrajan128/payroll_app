<template>
  <div class="main-20">
    <div flat color="white" class="row pb-8 pl-5 align-items-center">
      <span class="hrspannotice">{{
        $t(
          "the_final_process_by_hr_emp_exiting_make_sure_all_items_related_to_oraganisation_are_collected_back"
        )
      }}</span>
    </div>
    <v-form ref="answerform" v-model="valid">
      <v-data-table :headers="headers" :items="questions_array" :loading="initval" hide-default-footer disable-pagination>
            ...
            <td>
              <div v-if="item.field_type == 'LOV'" class="lov_autocomplete_div">
                <v-select v-if="item.is_field_required == 'Yes'" :rules="fieldRules" class="required_field"
                  v-model="questions_array[index].answer" :readonly="checksubmitdone" v-bind:label="$t('select')"
                ...
                <v-select v-else v-bind:label="$t('select')" :items="item.lookup_values" dense outlined
                  v-model="questions_array[index].answer" :readonly="checksubmitdone"></v-select>
              </div>
              <div v-if="item.field_type == 'AUTOCOMPLETE'" class="lov_autocomplete_div">
                <v-autocomplete v-if="item.is_field_required == 'Yes'" v-model="questions_array[index].answer"
                  :readonly="checksubmitdone" :rules="fieldRules" class="required_field" v-bind:label="$t('select')"
              ...
              <div v-if="item.field_type == 'TEXTAREA'" class="lov_autocomplete_div">
                <v-textarea v-if="item.is_field_required == 'Yes'" v-model="questions_array[index].answer"
                  :readonly="checksubmitdone" :rules="fieldRules" class="required_field" v-bind:label="$t('type_here')"
                  rows="2" dense outlined></v-textarea>
                <v-textarea v-else v-bind:label="$t('type_here')" rows="2" dense outlined
                  v-model="questions_array[index].answer" :readonly="checksubmitdone"></v-textarea>
              </div>
              <div v-if="item.field_type == 'TEXTFIELD'" class="lov_autocomplete_div">
                <v-text-field v-if="item.is_field_required == 'Yes'" v-model="questions_array[index].answer"
                  ...
                  :readonly="checksubmitdone"></v-text-field>
                <v-text-field v-else :readonly="checksubmitdone" v-bind:label="$t('type_here')" dense outlined
                  v-model="questions_array[index].answer"></v-text-field>
              </div>
              <div v-if="item.field_type == 'RADIO BUTTON'">
                <div v-if="item.is_field_required == 'Yes'">
                  <v-radio-group v-model="questions_array[index].answer" :readonly="checksubmitdone" :rules="fieldRules"
                    class="required_field" row>
                ...
                <div v-else>
                  <v-radio-group v-model="questions_array[index].answer" :readonly="checksubmitdone" row>
                    <v-radio class="vradiogroupvfor text--primary" v-for="(lookupchild, checkindex) in item.lookup_values"
                      :key="checkindex" :label="lookupchild" :value="lookupchild">
                    </v-radio>
                  </v-radio-group>
              ...
              <div v-if="item.field_type == 'RANGESLIDER'" class="lov_autocomplete_div">
                <v-range-slider hide-details class="align-center pt-2" step="1" thumb-size="22" thumb-label="always"
                  :readonly="checksubmitdone"></v-range-slider>
              </div>
            </td>
          </tr>
        </template>
      ...
      <div class="d-block text-right mr-4">
        ...
        <v-btn small @click="routeToexitInterview" class="primary mr-2">
          {{ $t("previous") }}
        </v-btn>
        <v-tooltip bottom v-if="!checksubmitdone && !finishLoader">
          <template v-slot:activator="{ on }">
            <div v-on="on" class="d-inline-block">
              <v-btn :disabled="isBtnLoading" color="green darken-1" small class="mr-2 success hover_shine"
                @click="saveHranswers">
                {{ $t("save") }}
        ...
        <span v-if="checksubmitdone == true">
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-btn small v-on="on" class="mr-2 ml-1 primary" :disabled="isBtnLoading1" @click="routeToFinal">{{
                $t("next") }}
  ...
  methods: {
    ...
    saveHranswers() {
      if (this.$refs.answerform.validate()) {
        ...
        axios
          .post(
            process.env.VUE_APP_API_URL_ADMIN + "saveemployeeanswerfromhr",
            {
    ...
    FinishHranswers() {
      if (this.$refs.answerform.validate()) {
        ...
        axios
          .post(
            process.env.VUE_APP_API_URL_ADMIN + "finishemployeeanswerfromhr",
            {
              answer: this.questions_array,
              empID: this.user_id,
            }
    ...
    fetchHRQuestions() {
      ...
      axios
        .get(
          ...
          "fetchhrquestionsforexitemployee/" +
          this.user_id
        )
        .then((res) => {
          if (res.data.status == "S") {
