        ...
        <tr v-bind:class="[props.item.empno == null ? 'back_color' : '']">
          <td>
            <router-link
              ...
              :to="{
                name: 'users.view',
                query: { slug: props.item.slug, view: 'userview' },
              }"
            >
              <v-tooltip bottom>
                <template v-slot:activator="{ on }"
                  ><v-chip v-on="on" color="warning" small outlined medium>
              ...
              <div class="widget-content-wrapper">
                <div class="widget-content-left mr-3">
                  <div
                    ...
                    v-if="props.item.profile_pic"
                  >
                    <img
                      width="36"
                      class="rounded-circle"
                      :src="envImagePath + props.item.profile_pic"
                      alt
                    />
                  </div>
                ...
                <div class="widget-content-left flex2">
                  <div class="widget-heading name-color">
                    {{ props.item.firstname }} {{ props.item.lastname }}
                  </div>
                  <div
                    class="widget-subheading opacity-7 name-color"
                    v-if="props.item.designation.designation"
                  >
                    {{ props.item.designation.designation }}
                  </div>
                </div>
              </div>
            </div>
          ...
          <td>
            <span v-if="props.item.designation_id == null">
              {{ $t("not_appllicable") }}</span
            >
            <span v-else>{{ props.item.designationdetails.designation }}</span>
          </td>
          <td>
            {{ props.item.gender }}
          </td>
          <td v-if="props.item.user_org == null">
            {{ $t("not_appllicable") }}
          </td>

          <!-- <td>
            <span v-if="props.item.level_id == null"> {{ $t("not_appllicable") }}</span>
            <span v-else>{{ props.item.levelidname.level_id }}</span>
          </td> -->
...
export default {
  ...
  methods: {
    ...
    fetchActiveUsers() {
      this.initval = true;
      axios
        .get(process.env.VUE_APP_API_URL_ADMIN + "fetchinactiveuser", {
          params: {
