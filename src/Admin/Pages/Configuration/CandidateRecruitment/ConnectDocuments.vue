        ...
        <v-hover v-slot="{ hover }" open-delay="0">
          <v-card class="maindocumentdiv mb-1" :elevation="hover ? 10 : 2">
            ...
            <div class="downloadbuttondocument" v-if="hover && doc.fileurl">
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <a v-on="on" class="text-center" :href="document_url + doc.fileurl" target="_blank">
                    <v-icon>
                      mdi-download
                    </v-icon>
                  </a>
    ...
    <v-dialog v-model="upload_dialog" persistent max-width="600px">
      <v-toolbar color="primary">
        <span class="headline" style="color: white" v-if="showalledit">{{
          $t("create_connect_document")
        }}</span>
        <span class="headline" style="color: white" v-else>{{
          $t("rename_connect_document")
        }}</span>
        <v-spacer></v-spacer>
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
        ...
        <v-form ref="form" v-model="valid">
          <v-layout pt-2 class="px-4">
            <v-flex xs12 md12 pl-2>
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-text-field v-on="on" v-model="document.document_name" maxlength="100" v-bind:label="$t('doc_name')"
          ...
          <v-layout pt-2 class="px-4" v-if="showalledit">
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-text-field readonly outlined dense v-on="on" @click="callDocumentSelect" :rules="fieldRules"
                      v-model="document.fileurl" v-bind:label="$t('upload_doc')"></v-text-field>
            ...
            <v-flex xs12 md4 pl-2 style="display: none">
              <DocumentSelect :label="'File'" :folder="'ConnectDocument'" :uploadtype="enable_document_select"
                @closeDialog="closeDocumentSelectDialog" @uploadedDocument="uploadedDocument"
                @uploadedFileType="uploadedFileType" :documentlink="document.fileurl"></DocumentSelect>
            </v-flex>
          </v-layout>
...
export default {
  ...
  watch: {
    connect_id: {
      immediate: true,
      handler() {
        this.document.connect_id = this.connect_id;
        this.fetchDocuments();
      },
    },
    menuclick: {
  ...
  methods: {
    searchDoc(name) {
      ...
      for (var i = 0; i < this.documentarray.length; i++) {
        var doc_name = (this.documentarray[i].document_name).toLowerCase();
    ...
    uploadedDocument(file_path) {
      this.loader = true;
      this.document.fileurl = file_path;
    ...
    submit() {
      if (this.$refs.form.validate()) {
        ...
        axios
          .post(
            process.env.VUE_APP_API_URL_ADMIN + "create_connect_document",
            this.document
          )
          .then((res) => {
    ...
    fetchDocuments() {
      this.loader = true;
      axios
        .get(process.env.VUE_APP_API_URL_ADMIN + "getconnectdocuments", {
          params: {
            connect_id: this.connect_id,
          },
        })
        .then((res) => {
          this.documentarray = res.data.documents;
          this.dummyarray = res.data.documents;
          this.loader = false;
        })
        .catch((err) => {
          this.flashMessage.error({
